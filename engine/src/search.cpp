...
static int move_score(const Board& board, const Move& move) {
    ...
    if (captured != 0) {
        int movingValue = piece_value(abs_piece(board.squares[move.fromRow][move.fromCol]));
        score += 10000 + piece_value(abs_piece(captured)) * 16 - movingValue;
    }
    if (move.promotion != 0) score += 8000 + piece_value(move.promotion);
    if (move.isCastling) score += 50;
    return score;
}

static void order_moves(const Board& board, std::vector<Move>& moves, const Move* preferred) {
    std::sort(moves.begin(), moves.end(), [&](const Move& a, const Move& b) {
        int sa = move_score(board, a), sb = move_score(board, b);
        if (preferred) {
            if (a == *preferred) sa += 1000000;
            if (b == *preferred) sb += 1000000;
        }
        return sa > sb;
    });
}
    ...
    if (ttEntry) {
        ...
        if (ttEntry->depth >= depth) {
            ...
            switch (ttEntry->flag) {
                ...
                case TTFlag::LowerBound: if (s >= beta) return s; alpha = std::max(alpha, s); break;
                case TTFlag::UpperBound: if (s <= alpha) return s; beta = std::min(beta, s); break;
            }
        }
    }
...
void move_to_uci(const Move& move, char out[6]) {
    ...
    out[2] = 'a' + move.toCol;
    out[3] = '1' + move.toRow;
    if (move.promotion != 0) { out[4] = promotion_char(move.promotion); out[5] = '\0'; }
    else out[4] = '\0';
}
...
Move search_best_move(Board& board, int depth, std::vector<BoardState>& stateStack) {
    ...
    Move bestMove = moves.front();
    if (TTEntry* e = probe_tt(board.hash)) bestMove = e->bestMove;
    ...
    for (int d = 1; d <= depth; ++d) {
        ...
        for (const Move& move : moves) {
            ...
            if (score > best) { best = score; iterBest = move; }
            if (score > alpha) alpha = score;
        }
